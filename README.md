...
- 🎤 **歌词显示** - 支持 LRC 歌词、全屏歌词与桌面歌词（毛玻璃 / 锁定穿透）；本地音乐可批量在线匹配 / 重新匹配；全屏播放页可在线匹配并多候选选择；歌词与歌声不同步时可手动校准偏移（每曲独立，实时生效并持久化）
- 🖼️ **匹配封面** - 列表右键自动匹配 / 重新匹配；全屏播放多候选选图；本地音乐可批量匹配无封面歌曲（MP3 写 Tag，非 MP3 仅更新应用内封面）
...
### v1.2.3 更新内容
- **匹配封面** - 在线匹配专辑封面；列表右键自动匹配，全屏播放多候选选择；本地音乐批量匹配无封面歌曲；MP3 写入文件 Tag，非 MP3 仅更新应用内封面
...
- **匹配歌词**：本地音乐页点「批量匹配歌词」为全部无歌词歌曲在线下载同名 `.lrc`；有歌词的歌曲右键「重新匹配歌词」；全屏播放控制栏也可「在线匹配」并多候选选择，点选候选即可预览该结果的歌词全文再决定是否使用
- **匹配封面**：本地音乐页点「批量匹配封面」为全部无有效封面歌曲在线匹配；列表右键「匹配/重新匹配封面」自动写入；全屏播放控制栏可「在线匹配封面」并多候选预览选图（也可选本地图片；不设相似度下限，可取消）；与批量歌词互斥不可同时进行

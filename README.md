...
- 🎵 **本地音乐管理** - 智能扫描、设置扫描目录、元数据编辑、封面提取；主窗口默认进入本地音乐
- 📁 **歌单管理** - 创建、编辑、设置封面、复制歌曲到目录、导入导出
- ❤️ **收藏功能** - 快速收藏喜欢的音乐
- 🎧 **高品质播放** - 原生 Audio 为主（Howler.js 兜底），支持多种音频格式；播放模式含列表顺序 / 随机 / 列表循环 / 单曲循环，上一首/下一首严格跟随所选模式；重启后从曲首播放（不保存进度，减少写库）
- 🎛️ **均衡器** - 内置多种预设音效，可自定义调节；全屏 / 迷你 / 普通模式共用同一滤波链
- 🔍 **智能搜索** - 支持歌曲、艺术家、专辑多维度搜索
- 🎤 **歌词显示** - 支持 LRC 歌词、全屏歌词与桌面歌词（毛玻璃 / 锁定穿透）；本地音乐可批量在线匹配 / 重新匹配；全屏播放页可在线匹配并多候选选择
- 🏷️ **智能标签编辑** - 智能解析文件名修复乱码标签；支持歌手/歌名对调；歌名/歌手/专辑/年份/流派可手动编辑，并支持单字段或整包 GB2312/GBK 转换
- 🗄️ **同步到数据库** - 仅写库不改文件，快速修复列表乱码（支持单曲与批量；英文标签防误转码）
- 📝 **文件名显示** - 列表显示文件名，便于识别和管理
- 🎨 **主题切换** - 浅色/深色/跟随系统（全屏播放页同步适配）
- 🖥️ **全屏播放** - 霓虹频谱 / 蓝色火焰 / 闪电 / 经典唱片 / CD / 磁带六套特效可切换；顶栏可单独关闭频谱类动画（唱盘类下开关禁用占位）；重启后记住上次选择
- 📱 **迷你模式** - 简洁迷你播放器；当前歌词、主题切换；封面可在无特效 / 唱片 / CD / 磁带间切换，并与全屏特效双向同步
- ⚡ **极致性能** - 虚拟滚动+分批加载，轻松应对万级曲库与万级播放队列
- 🖥️ **跨平台** - 支持 macOS（Apple Silicon）、Windows、Linux

## 🎯 最新版本 v1.2.2

### v1.2.2 更新内容
- **播放栏定位当前** - 音量按钮前增加「定位当前」，把正在播放的歌曲快速滚动/居中到本地音乐列表；若当前歌曲不在本地库，按钮会禁用并给出悬停提示
- **全屏队列右键菜单** - 全屏播放页显示播放队列时可右键：播放 / 收藏 / 加歌单 / 编辑标签 / 打开目录 / 详情 / 移出队列（不含批量操作）
- **编辑标签误触修复** - 编辑标签弹窗不再因点击遮罩/外部区域而关闭，避免未保存内容丢失

### v1.2.1 更新内容
- **CD / 磁带特效** - 全屏与迷你支持 CD、实体磁带封面特效；磁带开口朝上，贴纸区铺满专辑封面
- **特效双向同步** - 频谱/火焰/闪电进迷你为无特效并退出还原；迷你切到唱片/CD/磁带回全屏同步
- **顶栏特效开关** - 唱盘类模式下禁用占位，避免相邻按钮跳动
- **默认封面** - 去掉圆形外沿半透明软边色晕
- **音效链路** - 迷你与普通/全屏共用滤波链保活；离开全屏且未开音效时释放仅为频谱挂上的 Web Audio

### v1.2.0 更新内容
- **图标按钮 tips** - 无文字按钮补充悬停提示（清除搜索、窗口控件、迷你播放器、全屏顶栏、弹窗关闭等）
- **Windows 托盘图标** - 修复托盘空白：使用打包内置图标并按平台缩放，Windows 不再误用 macOS 模板图
- **Release 单实例** - 已打包版本同系统仅允许一个进程，二次启动聚焦已有窗口；Dev 可多开且与 Release 互不干扰
- **智能推荐** - 修复搜索历史被「高级搜索」污染；「随机播放」可打乱全部本地音乐并开始播放

### v1.1.9 更新内容
- **标签编辑增强** - 年份/流派可手动编辑；支持单字段或整包 GB2312/GBK 转换
- **音效链路修复** - 均衡器开关与滤波链可靠挂接；关闭时旁路、特效与 EQ 协同不再双重重建
- **清除所有** - 清空除设置与扫描目录外的库表；清库前停播并释放 Web Audio；失败可恢复队列；清空后按钮状态正确
- **本地音乐体验** - 默认进入本地音乐页；空列表禁用「播放全部」；无目录禁用「扫描」；「设置扫描目录」无目录时高亮引导
- **减少写库** - 播放进度不再实时落盘，重启后从曲首开始；仍保留队列、当前曲、音量与播放模式

### v1.1.8 更新内容
- **特效开关** - 全屏播放顶栏可单独开启/关闭频谱、火焰、闪电背景动画（唱片特效不受影响）
- **万级队列性能** - 全屏页与播放队列面板改为虚拟滚动，避免大歌单下 CPU 常高
- **Web Audio 按需释放** - 关闭特效且未开均衡器时恢复原生 Audio 直出，减少后台空转
- **状态持久化优化** - 队列与位置状态分离保存；失效队列曲目启动时自动清理

### v1.1.7 更新内容
- **播放模式切歌** - 上一首/下一首严格跟随模式：随机两端均随机；顺序到头/尾不切换；列表循环首尾互绕；单曲循环重播当前
- **播放失败跳过** - 损坏文件不再在单曲循环下死循环重试，改为跳过当前曲
- **Electron 国内镜像** - 默认经 npmmirror 下载 Electron / electron-builder 二进制
- **Windows 无声修复** - 移除导致部分音频设备静默失败的启动缓冲参数

### v1.1.6 更新内容
- **修复全屏封面底部被裁切** - 矮窗口下封面等比缩小，不再被 overflow 切掉下边
- **蓝色火焰跟频谱** - 分频带映射喷发位置与高度，上升快回落慢，贴近霓虹频谱的节拍感
- **新增闪电特效** - 分频带突增触发折线闪电；浅色深电弧 / 深色亮电光，无闪屏背景
- **批量匹配歌词** - 本地音乐一键为全部无歌词歌曲在线匹配；有歌词右键可重新匹配
- **全屏匹配歌词** - 控制栏可在线匹配；多结果弹窗选择；已有歌词需确认替换

### v1.1.5 更新内容
- **全屏播放浅色主题** - 跟随全局主题；封面取色按深浅分流；歌词/队列文字描边可读
- **四套全屏特效** - 霓虹频谱、蓝色火焰、闪电、经典唱片（密纹黑胶 + 镂空轴孔 + SVG 唱臂）；顶栏一键循环切换并持久化
- **全屏布局优化** - 封面+标题 | 歌词/队列 对半分栏；顶栏增加主题/语言切换
- **音质与频谱** - 未开音效时音频直通；均衡器设置防丢失；频谱去光晕、减卡顿并适配浅色
- **播放控件细节** - 进度条拖动钮浅色可见；去掉播放按钮阴影残影与多余分隔线
...
- **macOS**（仅 Apple Silicon / arm64）: `xmmusic-1.2.1-arm64.dmg`
- **Windows**: `xmmusic Setup 1.2.1.exe`
- **Linux**: `xmmusic-1.2.1.AppImage`
...
- **匹配歌词**：本地音乐页点「批量匹配歌词」为全部无歌词歌曲在线下载同名 `.lrc`；有歌词的歌曲右键「重新匹配歌词」；全屏播放控制栏也可「在线匹配」并多候选选择
- **编辑标签**：右键「编辑标签」可改歌名/歌手/专辑/年份/流派；右侧可对单字段点 GB2312/GBK 转换，或整包转换后逐字段应用；再「同步到数据库」或「保存」写回文件
- **播放模式**：播放栏循环切换「列表顺序 / 随机播放 / 列表循环 / 单曲循环」；上一首/下一首与自动切歌均跟随当前模式（顺序到头不切、列表循环首尾互绕、随机避开当前、单曲循环重播本曲）
- **同步到数据库**：列表显示乱码但文件本身正确时，右键「编辑标签」→「同步到数据库」，或批量操作栏批量同步（只改数据库，不改文件 ID3；英文标签不会被误转码）
- **主题**：顶部栏、设置页或全屏播放顶栏切换浅色 / 深色 / 跟随系统
- **全屏播放特效**：进入全屏播放页 → 顶栏特效按钮循环切换「霓虹频谱 / 蓝色火焰 / 闪电 / 经典唱片 / CD / 磁带」；「开启/关闭特效」可单独关掉频谱类背景动画（唱片/CD/磁带下该按钮禁用占位）；选择会记住
- **迷你封面样式**：迷你窗口右上角可切换「无特效 / 唱片 / CD / 磁带」；从全屏进入时按规则同步（频谱类 → 无特效；唱盘类原样带入），退出迷你时再写回全屏特效
- **音效与听感**：打开均衡器后，普通 / 迷你 / 全屏共用同一套 Web Audio 滤波链；关闭音效时尽量原生 Audio 直出（全屏频谱类特效为读频谱会旁路挂图，不经过均衡滤波）
...
│   │   ├── components/       # 通用 UI 组件 (Button, Modal、effects 特效等)

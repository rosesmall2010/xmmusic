...
- 🎵 **本地音乐管理** - 智能扫描、设置扫描目录、元数据编辑、封面提取；可清理磁盘上已不存在的失效记录；主窗口默认进入本地音乐
- 📁 **歌单管理** - 创建、编辑、设置封面、复制歌曲到目录
- ❤️ **收藏功能** - 快速收藏喜欢的音乐
...
- 🔍 **智能搜索** - 顶栏搜索支持汉字、全拼、声母（如 zjl / qt）
- 🎤 **歌词显示** - 支持 LRC 歌词、全屏歌词与桌面歌词（毛玻璃 / 锁定穿透）；本地音乐可批量在线匹配（最多 3 路并发）；列表右键与全屏均先显示拉取提示再多候选预览挑选；无在线结果时可关联同目录 `.lrc`；歌词与歌声不同步时可手动校准偏移（每曲独立，实时生效并持久化）
- 🖼️ **匹配封面** - 手动按标签歌名/歌手搜索，并叠加文件名推测差异词后合并排序；列表右键与全屏均先显示拉取提示再多候选预览选图（也可选本地图片）；本地音乐可批量匹配无封面歌曲（标签歌名≥50 → 歌手≥50 → 文件名推测再试；最多 3 路并发，与批量歌词互斥；MP3 写 Tag，非 MP3 仅更新应用内封面）
- 🏷️ **智能标签编辑** - 智能解析文件名修复乱码标签；支持歌手/歌名对调；歌名/歌手/专辑/年份/流派可手动编辑；ID3 乱码优先「自动识别」（UTF-8 / GB18030 / Latin1 / Big5），识别不准时可用 GBK 兜底
...
### v1.2.5 更新内容
- **ID3 自动识别编码** - 扫描入库与标签编辑改用自动识别（UTF-8 / GB18030 / Latin1 乱码还原 / Big5），按字段独立采纳；编辑弹窗以「自动识别」为主，保留 GBK 手动兜底
- **万级队列切歌定位** - 全屏播放队列与侧栏抽屉切歌（尤其随机模式）改为瞬时跳转，避免长距离 `smooth` 滚动
- **数据存放目录** - README 补充正式版 / 开发版在 macOS、Windows、Linux 下的 `userData` 路径与 `m4.db` 说明
- **稳定性** - 合入代码审查中/低优先级修复（歌单统计、路径 NFC、批量元数据进度、播放队列与主题响应等，详见 CHANGELOG）

### v1.2.4 更新内容
- **匹配拉取遮罩主题** - 修复浅色/深色下进度卡片对比度；去掉重复 `#app`，主题同步到 `<html>`，Teleport 也能继承变量
- **封面匹配策略** - 手动标签双搜 + 文件名推测差异词并按相似度排序（不设下限）；批量标签歌名 ≥50 → 歌手 ≥50 → 文件名推测再试，仍无则放弃
- **清理失效记录** - 本地音乐页可检查并删除磁盘上已不存在的曲库记录；同步清理歌单/收藏/最近播放/队列/发现列表，重算歌单统计，并修剪播放队列

### v1.2.3 更新内容
- **匹配封面** - 在线匹配专辑封面；列表右键与全屏均先显示「正在拉取」再多候选预览选图（也可选本地图片；无在线结果仍可选本地图）；本地音乐批量匹配无封面歌曲（最多 3 路并发，进度可取消，与批量歌词互斥）；自动/批量相似度 ≥50，手动候选不设下限；MP3 写入文件 Tag，非 MP3 仅更新应用内封面
- **匹配歌词体验对齐** - 列表右键同样弹出候选列表（可预览）；拉取期有可取消加载提示；无在线候选时关联同目录 `.lrc`；批量最多 3 路并发；自动/批量相似度 ≥50
- **拼音/声母搜索** - 顶栏搜索支持全拼与声母；FTS 与拼音旁路分流合并
- **应用内快捷键** - Space 播停、切歌、音量、收藏；设置页可自定义；不影响其他 App

### v1.2.2 更新内容
- **歌词偏移校准** - 全屏播放页歌词区右键菜单新增偏移校准：前进/后退 0.5/1/2 秒与「重置」，另有「在线匹配歌词」与「显示/隐藏歌词时间」（逐行显示精确到 0.1 秒的时间戳，便于核对校准效果）；歌词与歌声相差几秒时可实时校准，按每首歌独立持久化，迷你/桌面歌词窗口一并生效
...
- **新标签信息** - 三栏对比数据库 / 当前ID3 / 编辑后的歌手歌名专辑年份流派，支持逐字段或整包 GB2312/GBK 转码、复制ID3、复制数据库，另有对调歌手歌名、按文件名推测、重置；保存时 MP3 写回文件并同步数据库，非 MP3 仅同步数据库；MP3 文件可点「仅同步数据库」只改库不改文件；右键「编辑标签」现已改为打开此弹窗
...
- **匹配歌词**：本地音乐页点「批量匹配歌词」为全部无歌词歌曲在线下载同名 `.lrc`（最多 3 路并发）；列表右键「匹配/重新匹配歌词」与全屏播放均先显示拉取提示再打开多候选列表，可预览歌词全文后决定是否使用
- **匹配封面**：本地音乐页点「批量匹配封面」为全部无有效封面歌曲在线匹配（最多 3 路并发，进度条可看）；列表右键「匹配/重新匹配封面」先显示「正在拉取封面候选」再弹出候选列表预览选图（也可选本地图片）；全屏播放控制栏同样可「在线匹配封面」；与批量歌词互斥不可同时进行
- **拼音搜索**：顶栏输入全拼（如 zhoujielun）或声母（如 zjl）即可搜中文歌名/歌手
- **应用内快捷键**：设置 → 应用内快捷键；默认 Space 播停；仅在 XMMusic 窗口前台生效
- **歌词校准**：全屏播放页歌词区右键弹出菜单，选择「前进/后退 0.5/1/2 秒」即可让歌词对齐歌声，或点「重置」还原；勾选「显示歌词时间」可在每句歌词前看到精确到 0.1 秒的时间戳，随校准实时变化，方便对照效果；有歌词时才可用，无歌词时校准项禁用但「在线匹配歌词」始终可点；偏移按每首歌独立保存，迷你与桌面歌词同步生效
- **编辑标签**：右键「编辑标签」打开三栏对比（数据库 / 当前ID3 / 编辑后）；每字段可「自动识别」编码（推荐）或「GBK」兜底，也可「复制ID3」「复制数据库」填入编辑后列，并有对应批量按钮；另有「对调歌手歌名」「按文件名推测」（只改编辑后列）与「重置」；点「保存」时 MP3 写回文件标签并同步数据库，非 MP3 仅同步数据库；MP3 文件也可点「仅同步数据库」只改库不改文件 ID3

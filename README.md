...
- 🖥️ **全屏播放** - 霓虹频谱 / 蓝色火焰 / 闪电 / 经典唱片四套特效可切换，重启后记住上次选择
- 📱 **迷你模式** - 简洁的迷你播放器窗口
...
- **macOS**（仅 Apple Silicon / arm64）: `xmmusic-1.1.7-arm64.dmg`
- **Windows**: `xmmusic Setup 1.1.7.exe`
- **Linux**: `xmmusic-1.1.7.AppImage`
...
- **全屏播放特效**：进入全屏播放页 → 顶栏特效按钮循环切换「霓虹频谱 / 蓝色火焰 / 闪电 / 经典唱片」；选择会记住，下次启动仍是上次特效
...
2.  **Electron 国内镜像**（已内置）:
    *   项目通过 `.npmrc` 的 `electron_mirror` / `electron_builder_binaries_mirror`（以及打包脚本里的 `electron-mirror-env.js`）默认使用 [npmmirror](https://npmmirror.com/mirrors/electron/) 下载二进制，减轻国内访问 GitHub 过慢的问题
    *   作用于 `npm install`、`postinstall` 重建原生模块、以及 `npm run dist` / `dist:mac` / `dist:win` 打包
    *   npm 11+ 可能提示 Unknown project config（可忽略）；若需临时改回官方源：启动前设置环境变量覆盖，例如 `ELECTRON_MIRROR=https://github.com/electron/electron/releases/download/`

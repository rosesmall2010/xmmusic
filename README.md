...
### v1.2.2 更新内容
- **歌词偏移校准** - 全屏播放页歌词面板新增偏移滑块（±30 秒，0.5 秒步进）与「归零」；歌词与歌声相差几秒时可实时校准，按每首歌独立持久化，迷你/桌面歌词窗口一并生效
...
- **新标签信息** - 右键菜单新增独立入口，三栏对比数据库 / 当前ID3 / 编辑后的歌手歌名专辑年份流派，支持逐字段或整包 GB2312/GBK 转码、复制ID3、复制数据库，另有对调歌手歌名、按文件名推测、重置；保存时 MP3 写回文件并同步数据库，非 MP3 仅同步数据库，与「编辑标签」互不影响
...
- **歌词校准**：全屏播放页歌词面板底部拖动「歌词偏移」滑块（±30 秒，0.5 秒一档）即可让歌词对齐歌声；正值歌词提前、负值延后，点「归零」还原；偏移按每首歌独立保存，迷你与桌面歌词同步生效
- **编辑标签**：右键「编辑标签」可改歌名/歌手/专辑/年份/流派；右侧可对单字段点 GB2312/GBK 转换，或整包转换后逐字段应用；再「同步到数据库」或「保存」写回文件
- **新标签信息**：右键「新标签信息」打开三栏对比（数据库 / 当前ID3 / 编辑后）；每字段可单独「GB2312转码」「GBK转码」「复制ID3」「复制数据库」填入编辑后列，也有对应批量按钮；另有「对调歌手歌名」「按文件名推测」（只改编辑后列）与「重置」；保存时 MP3 写回文件标签并同步数据库，非 MP3 仅同步数据库

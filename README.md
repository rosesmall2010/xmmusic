...
- 🗄️ **同步到数据库** - 仅写库不改文件，快速修复列表乱码（支持单曲与批量）
...
功能说明详见 [docs/1.1.3/README.md](docs/1.1.3/README.md)
...
- **macOS**（仅 Apple Silicon / arm64）: `xmmusic-1.1.3-arm64.dmg`
- **Windows**: `xmmusic Setup 1.1.3.exe`
- **Linux**: `xmmusic-1.1.3.AppImage`
...
- **同步到数据库**：列表显示乱码但文件本身正确时，右键「编辑标签」→「同步到数据库」，或批量操作栏批量同步（只改数据库，不改文件 ID3）
- **主题**：顶部栏或设置页切换浅色 / 深色 / 跟随系统

更完整的 v1.1.3 功能说明见 [docs/1.1.3/README.md](docs/1.1.3/README.md)。

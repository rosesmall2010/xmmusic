...
- **编辑标签**：右键「编辑标签」打开三栏对比（数据库 / 当前ID3 / 编辑后）；每字段可单独「GB2312转码」「GBK转码」「复制ID3」「复制数据库」填入编辑后列，也有对应批量按钮；另有「对调歌手歌名」「按文件名推测」（只改编辑后列）与「重置」；点「保存」时 MP3 写回文件标签并同步数据库，非 MP3 仅同步数据库；MP3 文件也可点「仅同步数据库」只改库不改文件 ID3

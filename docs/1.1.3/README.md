...
**解决的问题**：部分歌曲在本地列表里标题/歌手显示乱码，但文件名和（MP3）文件内标签本身可解析正确。
...
| 批量 | 右键 →「批量操作」→ 勾选 →「同步到数据库」 | 确认后自动解析并批量写库 |

**批量解析规则**：

- **MP3**：优先读取 ID3；若疑似乱码会尝试编码检测/转换，仍乱码则降级为文件名解析
- **非 MP3**（FLAC/M4A 等）：当前**仅**按文件名解析（不读内嵌标签）
- **专辑**：无可靠来源时写空，不会用歌手名填专辑
...
| 锁定模式 | 锁定后去掉毛玻璃（macOS `setVibrancy(null)` / Windows `setBackgroundMaterial('none')`），变为纯透明悬浮文字（鼠标穿透） |
...
| `batchSyncMusicMetadataToDb(ids)` | 自动 `resolveMetadataForSync` 后批量写库：MP3 读 ID3（含编码纠正/乱码降级），非 MP3 仅文件名 |
